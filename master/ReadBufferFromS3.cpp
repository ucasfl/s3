...
bool ReadBufferFromS3::nextImpl()
{
    ...
    for (Int64 attempt = static_cast<Int64>(s3_max_single_read_retries); attempt >= 0; --attempt)
    {
        ...
        catch (const Exception & e)
        {
            ...
            auto * istr = dynamic_cast<ReadBufferFromIStream *>(impl.get());
            already_read += istr->already_read;
...
std::unique_ptr<ReadBuffer> ReadBufferFromS3::initialize()
{
    ...
    if (new_offset != static_cast<size_t>(getPosition()))
        LOG_DEBUG(log, "DIFF: getPosition: {}, new_offset:{}", getPosition(), new_offset);
